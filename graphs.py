...
class Graph:
    ...
    def do_graph(self):
        ...
        # plt.title(self.name)
        # if f_F and f_U:
        plt.annotate(f'$f_N$', xy=(68, 5.1),  xytext=(55, 5.5), size=15, arrowprops=dict(facecolor='black', shrink=0.1)) 
        plt.annotate(f'$f_U$', xy=(68, 3.9),  xytext=(58, 3), size=15, arrowprops=dict(facecolor='black', shrink=0.01)) 
        plt.legend()
        plt.xlim(5, 85)
        plt.ylim(1.5, 6)
        plt.show()
